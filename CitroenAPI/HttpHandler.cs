...
namespace ReptilApp.Api
{
    public class HttpLoggingHandler : DelegatingHandler
    {
        ...
        async protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            Debug.WriteLine($"{msg} HOST COMPLETE: {req.RequestUri}");
            ...
            if (req.Content != null)
            {
                ...
                if (req.Content is StringContent || this.IsTextBasedContentType(req.Headers) || this.IsTextBasedContentType(req.Content.Headers))
                {
                    ...
                    Debug.WriteLine($"{msg} {string.Join("", result.Cast<char>().Take(20555565))}...");

                }
                if (req.Content is MultipartContent)
                {
                    var multipartContent = (MultipartContent)req.Content;
